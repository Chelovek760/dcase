...
def get_machine_id_list_for_test_M(target_dir,
                                   dir_name="test",
                                   ext="json"):
    """
    target_dir : str
...
def test_file_list_generator_M(target_dir,
                               id_name,
                               dir_name="test",
                               prefix_normal="normal",
                               prefix_anomaly="anomaly",
...
if __name__ == "__main__":
    ...
    for idx, target_dir in enumerate(dirs):
        ...
        modelM = keras_model.load_model(model_fileM)
        model.summary()
        ...
        for id_str in machine_id_list:
            ...
            for file_idx, file_path in tqdm(enumerate(test_files), total=len(test_files)):
                print(file_path)
                try:
                    dataM = comM.file_to_vector_array(test_files_M[file_idx])
                    ...
                    errorsM = numpy.mean(numpy.square(dataM - modelM.predict(dataM)), axis=1)
                    # errors_matrix_for_class[file_idx,:]=errors
                    y_pred[file_idx] = numpy.mean(errors)
                    anomaly_score_list.append([os.path.basename(file_path), y_pred[file_idx], numpy.mean(errorsM)])
