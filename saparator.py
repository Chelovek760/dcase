...
files=[r'D:\Ботать\Работа\dcase\dev_data\dcase\fan\train\normal_id_00_00000001.wav']
DELITEL_COUNT=99
for num,file in tqdm(enumerate(files),total=len(files)):
    ...
    newtimeshape=wavlet.c_wavlet_coef.shape[1]//DELITEL_COUNT*DELITEL_COUNT
    print(wavlet.c_wavlet_coef.shape[1],newtimeshape)
    wavlet_list=np.hsplit(wavlet.c_wavlet_coef[:,:newtimeshape], DELITEL_COUNT)
    print(wavlet_list[0].shape[0]*wavlet_list[0].shape[1])
    X=np.zeros((DELITEL_COUNT,wavlet_list[0].shape[0]*wavlet_list[0].shape[1]))
    ...
    if countminus>DELITEL_COUNT//2:
        res=res*-1
    for i in np.argwhere(res==-1):
        x1,x2=wavlet.x_axis_time[i*newtimeshape//DELITEL_COUNT],wavlet.x_axis_time[(i+1)*newtimeshape//DELITEL_COUNT]
