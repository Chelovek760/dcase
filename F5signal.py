...
try:
    ...
    warnings.warn(
        "Can't import Audio from IPython.dispflay; " "Wave.make_audio() will not work."
    )
...
def razryad(data,n):
    """CHANGE ME PLZ
    """
    data=[data[i] for i in range(0,len(data),n)]
    return data

def razryad_2d(data,n,m):
    """CHENGE ME PLZ
    """
    new=np.zeros((len(range(0,np.shape(data)[0],n)),len(range(0,np.shape(data)[1],m))))
    for row in range(0,len(data),n):
        new[row]=[data[row][i] for i in range(0,len(data[row]),m)]
    return new
...
def play_wave(filename="sound.wav", player=r"C:\Program Files\Windows Media Player\wmplayer.exe", mode=r'play'):
    ...
    """
    cmd = '"%s" /%s "%s"' % (player, mode, os.getcwd() + '\\' + filename)
...
class Wave:
    ...
    def __init__(self, ys, ts=None, framerate=None):
        ...
        if ts is None and framerate is None:
            raise AttributeError ("We need to indicate either the frequency or the time series")

        self.framerate = framerate if framerate is not None else np.arange(len(ys)) / np.asanyarray(ts)
    ...
    def downscalefreq(self, n):
        """Down scale frequansis to n
        
        Return wave[::n] 
        """

        return Wave(self.ys[::n], self.ts[::n], framerate=int(self.framerate / n))
    ...
    def plot(self, **options):
        ...
        F5plot.plot(self.ts * xfactor, self.ys, **options)
    ...
    def plot_vlines(self, **options):
        ...
        """
        xfactor = self.get_xfactor(options)
        F5plot.vlines(self.ts * xfactor, 0, self.ys, **options)
...
class Spectrogram:
    ...
    def plot(self, high=None, **options):
        ...
        F5plot.pcolor(ts, fs, array, **options)
...
class _SpectrumParent:
    ...
    def plot(self, high=None, **options):
        ...
        if self.full:
            ...
            F5plot.plot(fs, amps, **options)
        else:
            i = None if high is None else find_index(high, self.fs)
            F5plot.plot(self.fs[:i], self.amps[:i], **options)
    ...
    def plot_power(self, high=None, **options):
        ...
        if self.full:
            fs, amps = self.render_full(high)
            F5plot.plot(fs, amps ** 2, **options)
        else:
            i = None if high is None else find_index(high, self.fs)
            F5plot.plot(self.fs[:i], self.power[:i], **options)
...
class Signal:
    ...
    def make_wave(self, duration, framerate, start=0):
        """Makes a Wave object.
